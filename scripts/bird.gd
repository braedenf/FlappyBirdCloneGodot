...
func set_state(new_state):
	...
	emit_signal("state_changed", self) #Signal a state change
	pass
...
class FlappingState:
	...
	func _init(bird):
		...
		#sets the x velocity of the bird to right, y stays the same
		bird.set_linear_velocity(Vector2(bird.speed, bird.get_linear_velocity().y))
		flap()
		pass

	func update(delta):
		#stops the rotation when the bird exceeds 30 degrees
		if rad2deg(bird.get_rot()) > 30:
			bird.set_rot(deg2rad(30))
			bird.set_angular_velocity(0) #ensures the bird dosn't over rotate
		
		#rotates the bird clockwise when falling
	...
	func flap():
		#Sets a new linear velocity on the y axis
		bird.set_linear_velocity(Vector2(bird.get_linear_velocity().x, -150))
		#sets the angular velocity anti-clockwise when flapped
